...
let backupStatusEl: HTMLElement | null = null;
let backupTimeEl: HTMLElement | null = null;
let downloadBackupBtn: HTMLButtonElement | null = null;
let generateBackupBtn: HTMLButtonElement | null = null;
let importBackupBtn: HTMLButtonElement | null = null;
let importFileInput: HTMLInputElement | null = null;
let createTokenBtn: HTMLButtonElement | null = null;
let tokenResultContainer: HTMLElement | null = null;
let tokenInput: HTMLInputElement | null = null;
let copyTokenBtn: HTMLButtonElement | null = null;
...
export function applyTheme(theme: string): void {
  ...
  if (modalWindow) {
    const cards = modalWindow.querySelectorAll<HTMLButtonElement>(".config_theme_card");
    cards.forEach((card) => {
...
async function fetchBackupInfo(): Promise<void> {
  if (!backupStatusEl || !backupTimeEl) return;
...
let confirmWindow: HTMLElement | null = null;
let confirmCancelBtn: HTMLButtonElement | null = null;
let confirmActionBtn: HTMLButtonElement | null = null;

function createConfirmModalDOM(): void {
...
function showImportConfirmation(payload: any): void {
  createConfirmModalDOM();
  if (!confirmBackdrop || !confirmCancelBtn || !confirmActionBtn) return;
  ...
  const newCancelBtn = confirmCancelBtn.cloneNode(true) as HTMLButtonElement;
  const newActionBtn = confirmActionBtn.cloneNode(true) as HTMLButtonElement;
...
async function handleCreateToken(): Promise<void> {
  if (!createTokenBtn || !tokenResultContainer || !tokenInput || !copyTokenBtn) return;
...
async function handleCopyToken(): Promise<void> {
  if (!tokenInput || !copyTokenBtn || !tokenInput.value) return;
...
function createConfigModalDOM(): void {
