...
let footerSaveBtn = null;
let toastEl = null;
let editorHolderEl = null;
...
let discardConfirmBackdrop = null;
...
function updateBadgeStatus(saved) {
    if (!badgeEl)
        return;
    if (saved) {
...
function initResizeHandler(handle, windowEl) {
...
function parseContentToEditorJSData(content) {
...
async function initEditorInstance(initialData) {
    ...
    const editorGlobal = window.EditorJS || (typeof EditorJS !== "undefined" ? EditorJS : null);
...
export async function saveAnnotation() {
    if (!editorInstance || !currentDetail)
        return false;
    try {
        const outputData = await editorInstance.save();
        const cleanContent = outputData.blocks.length === 0 ||
...
export async function openAnnotationModal(detail, onSave) {
    createModalDOM();
...
function createDiscardConfirmDOM() {
    if (discardConfirmBackdrop)
        return;
    ...
    discardConfirmBackdrop.addEventListener("mousedown", (e) => {
...
function promptDiscardConfirmation() {
...
export async function closeAnnotationModal() {
    if (!modalBackdrop?.classList.contains("active"))
        return;
    if (isUnsaved) {
...
export function isAnnotationModalOpen() {
    return Boolean(modalBackdrop?.classList.contains("active"));
}
export function getCurrentAnnotationDetailId() {
